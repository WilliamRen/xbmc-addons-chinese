...
def Roots():
	...
	li=xbmcgui.ListItem(u'爱布谷（即将上线）'.encode('utf8'))
	u=sys.argv[0]+"?mode=20&name="+urllib.quote_plus(u'爱布谷（即将上线）'.encode('utf8'))+"&url="+urllib.quote_plus('http://bugu.cntv.cn/')
	xbmcplugin.addDirectoryItem(int(sys.argv[1]),u,li,True)
	li=xbmcgui.ListItem(u'爱直播（即将上线）'.encode('utf8'))
	u=sys.argv[0]+"?mode=20&name="+urllib.quote_plus(u'爱直播（即将上线）'.encode('utf8'))
	xbmcplugin.addDirectoryItem(int(sys.argv[1]),u,li,True)
	...
	xbmcplugin.addDirectoryItem(int(sys.argv[1]),u,li,True)
        ...
        if category=='Video':
                ...
                for i in range(0,len(match0)):
                	li=xbmcgui.ListItem(str(i+1)+'.'+match0[i][8],'',match0[i][0])
                ...
                for i in range(0,len(match0)):
                	li=xbmcgui.ListItem(str(i+1)+'.'+match0[i][4],'',match0[i][0])
...
def XiyouF(name,id):
        ...
        for i in range(0,len(match)):
                li=xbmcgui.ListItem(str(i+1)+'.'+match[i][8],'',match[i][0])
...
def XiyouG(name,id,director,thumb,plot,duration):
        ...
        for i in range(0,len(match)):
                ...
                li=xbmcgui.ListItem(u'播放相关视频：'+eval('u"'+match[i][0]+'"'),'',match[i][5].replace('\\',''))
